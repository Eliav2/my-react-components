...
var XarrowPathShape = function (props) {
  ...
  var ps = new classes_1.Vector(posState.startPoint.x, posState.startPoint.y);
  var pe = new classes_1.Vector(posState.endPoint.x, posState.endPoint.y);
  ...
  // for 'middle' anchors
  if (startDir.size() === 0)
    startDir = new classes_1.Dir(
      ll.diff.abs().x > ll.diff.abs().y ? new classes_1.Vector(ll.diff.x, 0) : new classes_1.Vector(0, ll.diff.y)
    );
  if (endDir.size() === 0)
  ...
  var cp1 = new classes_1.Vector(ps),
    cp2 = new classes_1.Vector(pe);
  var cps1 = "";
  var cps2 = "";
  if (props.path === "straight") {
    getPathState = getPathState(
      ...
      function (pos) {
        return "M " + pos.startPoint.x + " " + pos.startPoint.y + " L " + pos.endPoint.x + " " + pos.endPoint.y;
      }
    );
  } else {
    ...
    getPathState = getPathState(function (pos) {
      return __assign(__assign({}, pos), { cp1: cp1, cp2: cp2 });
    });
    if (props.path === "grid") {
      ...
      getPathState = getPathState(
        ...
        function (pos) {
          return "M " + pos.startPoint.x + " " + pos.startPoint.y + " " + cps1 + " " + cps2 + " L " + pos.endPoint.x + " " + pos.endPoint.y;
        }
      );
  ...
  var child =
    (_d = (_c = props.children) === null || _c === void 0 ? void 0 : _c.call(props, getPathState)) !== null && _d !== void 0 ? _d : jsx;
  return (
    <>
      {child}
      {props._debug && (
